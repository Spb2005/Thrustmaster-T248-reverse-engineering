...
enum UartState {
  ...
  UART_RECV_42    // 0x42 screen/frame data
};
...
void loop() {
  ...
  // Reset momentary buttons after timeout
  if (btnpressed) {
    unsigned long now = millis();
...
void processF3Frame(uint8_t *buf) {
  ...
  // Example decoding of encoder/status bits
  if (buf[5] == 0x45) {
...
void handleCommand(String cmd) {
  ...
  // Immediately transmit button press (B0 only)
  sendButtons(0);
}
