...
void loop() {
  ...
  if (now - lastPeriodicSend >= PERIOD_MS) {
    ...
    sendButtons(1);  // Full transmission (B0 + B1)
  }

  // Check USB serial console for commands
  if (Serial.available()) {
...
void handleCommand(String cmd) {
  ...
  // Immediately transmit updated button state (B0 only)
  sendButtons(0);
}
...
void sendButtons(bool full) {
  ...
  Serial2.flush();
  delayMicroseconds(500);  // Inter-frame delay
